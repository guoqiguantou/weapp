...
Page({
  ...
  //点击保存按钮
  handleClick() {
    let { output, sales_time, unit_price, purchaser, remarks, product_name } = this.data.formdata;
    ...
    if (output == null || output == '' || output == ' ') {
    ...
    } else if (product_name == null || product_name == '' || product_name == ' ') {
      $Message({
        content: '请选择输入名称',
        type: 'error'
      });
      ...
      if (this.edit) {
        $request({
          url: app.globalData.serverBook + "Other_production_marketing/updateOther",
          method: "POST",
          type: 'form',
          params: {
            ...this.data.formdata,
        ...
        $request({
          url: app.globalData.serverBook + "Other_production_marketing/insertOther",
          method: "POST",
          type: 'form',
          params: {
            ...this.data.formdata
  ...
  getlistdata(id) {
    ...
    $request({
      url: app.globalData.serverBook + "Other_production_marketing/selectOtherById",
      method: "POST",
      type: 'form',
      params: {
        id: id
      ...
      if (res.data.resultCode == 0) {
        this.setData({
          ...
          ['formdata.product_name']: res.data.data.product_name,
