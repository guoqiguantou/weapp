...
Page({
  ...
  data: {
    ...
    currentRouter: {
      ...
      moneyById: serverList.serverWorker + "farmhand/findListBySid",
      workerInfo_url: serverList.serverWorker + "labor/findOneBywsId",
      holiday_url: serverList.serverWorker + "farmhand/findDays",
      add: {
        url: serverList.serverWorker + "farmhand/addFarmhand",
      },
      edit: {
        url: serverList.serverWorker + "labor/updLabor"
      }
    }
  },
  ...
  handleSubmit() {
    ...
    let currentRouter = that.data.currentRouter
    if (formData.adMoney.replace(/(^\s*)|(\s*$)/g, "") == ""){
      formData.adMoney = "0"
    }
